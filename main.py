		...
		if _quantity and _code and request.method == 'POST':
			...
			if 'cart_item' in session:
				...
				if row['code'] in session['cart_item']:
					...
					cursor.execute("SELECT * FROM cart WHERE product_code=%s", _code)
					cartrow = cursor.fetchone()
					cart_quant = cartrow['quantity']
					c.commit()
...
def merch_product_edit():
    return render_template('merch_product_edit.html')
...
def merchforgot():
	...
	try:		
		...
		if _name and _email and _password and request.method == 'POST':
			...
			return 'Error while updating information'
	except Exception as e:
		print(e)
	finally:
		cursor.close() 
