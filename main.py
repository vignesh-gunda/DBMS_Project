...
def checkoutcomp():
	...
	try:
		...
		headings = ("Product Name", "Product Code", "Individual Price", "Total Quantity")
		orders = ()
		...
		for row in rows:
			...
			orders = orders + ((_productname,_productcode,_productprice,_productquantity),)
...
@app.route('/merchant_delete_product/<int:id>')
def merchant_delete_product(id):
	_id = id
	_email = session['merchant_email']
	
	conn = mysql.connect()
	cursor = conn.cursor()
	sql_2 = "DELETE FROM product WHERE id=%s AND merchant_id = %s"
	data_2 = (_id, _email)
